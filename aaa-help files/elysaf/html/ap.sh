...
[ $# -eq 0 ] && { echo "Usage: ap.sh [wireless-device] [internet-device] [AP network name]"; exit 1; }
wifi="$1"
eth="$2"
control_c()
{
...
function cleanup() {
	...
	pkill hostapd
	echo -e "\e[32m>>>Stopping apache server"
	systemctl stop apache2
	echo -e "\e[32m>>>Restart network manager"
	service networking restart
	sleep 1
	service network-manager restart
	sleep 1
	echo -e "\e[32m>>>AP stopped!"
    exit $?
}
trap control_c SIGINT
echo -e "\e[32m>>>Stopping network manager"
