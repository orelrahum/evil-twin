...
## Possible errors
  ...
  * Error for "Set Frequency":  
    ...
    - You ran ```wifi_attack.py``` and ```defence.py``` at the same time, and you use the same network interface in 'monitor mode'. If one program ended, before the other, and it switched the network interface back to 'managed mode', the other program still trying to use the network interface in 'monitor mode'  
